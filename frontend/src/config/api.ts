...
if (process.env.NODE_ENV === 'production') {
  apiBaseUrl = process.env.REACT_APP_PROD_API_BASE_URL || 'http://10.0.1.226:8080'; // 生产环境API
} 
// Check for a specific custom environment variable for a staging/test API
  ...
  apiBaseUrl = process.env.REACT_APP_STAGING_API_BASE_URL || 'http://10.0.1.226:8080'; // 测试环境API
} 
// Default to development environment for all other cases
else {
  apiBaseUrl = process.env.REACT_APP_DEV_API_BASE_URL || 'http://10.0.1.226:8080'; // 开发环境API (你的FastAPI后端)
}
