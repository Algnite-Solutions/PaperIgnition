...
if (process.env.NODE_ENV === 'production') {
  apiBaseUrl = process.env.REACT_APP_PROD_API_BASE_URL || 'https://your-production-api.com/api'; // 生产环境API
} 
// Check for a specific custom environment variable for a staging/test API
  ...
  apiBaseUrl = process.env.REACT_APP_STAGING_API_BASE_URL || 'https://your-staging-api.com/api'; // 测试环境API
} 
// Default to development environment for all other cases
else {
  apiBaseUrl = process.env.REACT_APP_DEV_API_BASE_URL || 'http://localhost:8000/api'; // 开发环境API (你的FastAPI后端)
}
