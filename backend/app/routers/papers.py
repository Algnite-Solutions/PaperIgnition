...
# Request model for updating paper blog
class UpdatePaperBlogRequest(BaseModel):
    paper_id: str
    blog_content: str

class UpdatePapersBlogRequest(BaseModel):
    papers: List[UpdatePaperBlogRequest]

@router.put("/update-blog", status_code=status.HTTP_200_OK)
async def update_papers_blog(
    request: UpdatePapersBlogRequest,
    index_service_url: str = Depends(get_index_service_url)
):
    """Update blog field in papers table for multiple papers via index service"""
    try:
        import httpx
        
        # Prepare the request data for index service
        papers_data = []
        for paper in request.papers:
            paper_id = paper.paper_id
            blog_content = paper.blog_content
        ...
        if not papers_data:
            logger.warning("No valid papers to update")
            return {
                "message": "No valid papers to update",
                "updated_count": 0,
                "total_requested": len(request.papers)
            }
        
        # Call index service API to update papers blog field
        request_data = {"papers": papers_data}
        
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.put(
                f"{index_service_url}/update_papers_blog/",
                json=request_data
            )
            response.raise_for_status()
            
            result = response.json()
            logger.info(f"Index service response: {result}")
            
            return {
                "message": f"Successfully updated blog fields for {result.get('updated_count', 0)} papers",
                "updated_count": result.get('updated_count', 0),
                "total_requested": len(request.papers)
            }
        
    except httpx.HTTPError as e:
        logger.error(f"HTTP error when updating papers blog field: {str(e)}")
        raise HTTPException(status_code=500, detail=f"HTTP error when updating papers blog field: {str(e)}")
    except Exception as e:
